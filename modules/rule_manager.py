...
class RuleManager:
    ...
    def upsert_rules(self, df: DataFrame) -> None:
        ...
        null_check = df.select([col(c).isNull().alias(c) for c in df.columns]) \
               .agg(*(F.max(c).alias(c) for c in df.columns))

        # Show columns with null values
        columns_with_nulls = [col for col, is_null in null_check.first().asDict().items() if is_null]
    ...
    def update_rules(self, rules_tables: list):
    ...
    def _update_dim_rule_type(
        self, dim_rule_type_table: str = "dqops.brnz_sharepoint_ref_rule_types_ref_rule_types"
    ) -> None:
    ...
    def parse_rules(self, context_manager, rules: DataFrame = None) -> dict:
        ...
        for row in rules.toLocalIterator():
            ...
            if row is None or row['rule_enabled'] == 'N':
                print(f"Rule not enabled, skipping: {row['rule_key']}")
                continue
            ...
            if data_asset is None:
                context_manager.data_assets[data_asset_name] = DataAsset(catalog, schema, table_name)
            ...
            print(f"({row['rule_key']}) Adding expectation: {row['gx_expectation']} - Rule type key ({row['rule_type_key']})")
